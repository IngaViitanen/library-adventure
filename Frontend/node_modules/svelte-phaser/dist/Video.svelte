 ...
 */
export let postPipeline = undefined;
/**
 ...
 *
 */
export let load = undefined;
const dispatch = createEventDispatcher();
const scene = getScene();
export let instance = new Phaser.GameObjects.Video(scene, x, y);
onMount(() => {
    const videoEventListeners = [
        createPhaserEventDispatcher(instance, dispatch, 'unlocked', (video, error) => ({
            video,
            error,
        })),
        createPhaserEventDispatcher(instance, dispatch, 'error', (video, error) => ({
            video,
            error,
        })),
        createPhaserEventDispatcher(instance, dispatch, 'timeout', (video, error) => ({
            video,
            error,
        })),
        ...
        createPhaserEventDispatcher(instance, dispatch, 'stop', (video, width, height) => ({
            video,
            width,
            height,
        })),
...
$: if (shouldApplyProps(load === null || load === void 0 ? void 0 : load.url)) {
    instance.loadURL(load.url, load.event, load.noAudio);
}
$: if (shouldApplyProps(load === null || load === void 0 ? void 0 : load.stream)) {
